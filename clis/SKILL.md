...
| laptrack links everything after switching `mode`                        | cutoffs default per mode (overlap 0.9, centroid 225) only when the yaml leaves them unset; a generated yaml pins 0.9, so set `cutoff`/`gap_closing_cutoff` to 225 when switching to centroid                                                                                                        |
