...
export class FakeApi implements PixelApi {
  auto = true
  ...
  slice(q: SliceCall['q'], signal?: AbortSignal): Promise<PlaneBuffer> {
    ...
    const plane = makePlane(q.axis === 'xz' ? 3 : 3, 1024, q.cs.length, q.level)
  ...
  volume(q: VolumeCall['q'], signal?: AbortSignal): Promise<VolumeBuffer> {
    ...
    if (this.auto) return Promise.resolve(makeVolume(3, 256, 256, q.level))
